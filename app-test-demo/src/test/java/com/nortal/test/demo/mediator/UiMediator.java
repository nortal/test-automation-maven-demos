...
public class UiMediator {
    ...
    public ElementsCollection elementsXpath(String xpath, boolean highlightOverride) {

        if (testDemoProperties.isDemoMode() && highlightOverride) {
            demoMode(xpath, ElementType.XPATH);
        }

        ElementsCollection action = RetryingInvoker.retry(() -> Selenide.elements(By.xpath(xpath)));
        if (testDemoProperties.isScreenshotsSteps()) {
            takeScreenshot();
        }
